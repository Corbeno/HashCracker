...
test.describe('Hashing Flow', () => {
  ...
  test('should crack a simple MD5 hash', async ({ page }) => {
    ...
    await expect(page.getByText('Running').or(page.getByText('Queued')).first()).toBeVisible({ timeout: 10000 });
    
    // Wait for hash to be cracked and appear in Cracked Hashes panel
    // This should be fast with rockyou wordlist
    await expect(page.getByText('5f4dcc3b5aa765d61d8327deb882cf99:password')).toBeVisible({ timeout: 60000 });
    
    // Verify job shows completed status
    await expect(page.getByText('Completed').or(page.getByText('Exhausted')).first()).toBeVisible({ timeout: 65000 });
  });
  ...
  test('should crack multiple hashes', async ({ page }) => {
    ...
    await expect(
      page.getByText('5f4dcc3b5aa765d61d8327deb882cf99:password')
  ...
  test('should select different hash type', async ({ page }) => {
    ...
    await expect(page.getByText('Type: NTLM').or(page.getByText('Type: 1000'))).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should highlight cracked hashes in job panel', async ({ page }) => {
    ...
    const crackedHashInJob = jobPanel.locator('.text-green-400').filter({ hasText: '5f4dcc3b5aa765d61d8327deb882cf99' });
  ...
  test('should copy job hashes to input', async ({ page }) => {
    ...
    const jobPanel = page.locator('.bg-gray-900\\/50').filter({ hasText: '5f4dcc3b5aa765d61d8327deb882cf99' }).first();
