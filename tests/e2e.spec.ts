...
test.describe('Hash Cracker E2E', () => {
  ...
  test('should open Potfile modal', async ({ page }) => {
    ...
    // Potfile modal uses an SVG X button primarily, but checking implementation it might not have a text "Close" button?
    // Let's check PotfileModal.tsx again. It has a button with SVG in header.
    // It DOES NOT have a footer Close button in the file content `Showing lines 1 to 61`.
    // Wait, let's verify if I can find the SVG button by label.
    // The SVG button has no aria-label in the code I seeing.
    // <button onClick={onClose} className="text-gray-400 hover:text-white"> <svg ...> </button>
    // I should add aria-label to PotfileModal close button to make it testable, OR use a locator like `page.locator('button').filter({ has: page.locator('svg') })`.
    // But better to click the backdrop? No, pure backdrop click not implemented.
    // I will try to click the button by finding the header and then the button inside.
  ...
  test('should extract hashes in Yoink modal', async ({ page }) => {
    ...
    const modal = page.locator('div').filter({ has: page.getByRole('heading', { name: 'Yoink Hashes from your logs!' }) }).first();
